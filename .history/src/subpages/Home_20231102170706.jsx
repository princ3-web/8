...
const Home = () => {
  ...
  return (
    <Box>
      ...
      <Box sx={{ display: "flex", flexDirection:"column", alignItems: "center", justifyContent: "center", py: "4rem" }}>
        <Typography variant="h5" sx={{ width: "40%", textAlign: "center", fontWeight: 900 }}>
          Medical news and health news headlines posted throughout the day, every day.
        </Typography>
        <Box sx={{display:"flex"}}>
          {items1.map((item) => (
            <Box sx={{  position: "relative", display:"flex", alignItems:"cente" width:"100%" }}>
              <CardMedia
                component="img"
                src={item.image}
                alt=""
                sx={{ width: "12rem", height: "12rem", border:"1px solid red" }}
              />
              <Typography sx={{ position: "absolute", bottom: "1rem" }}>{item.title}</Typography>
            </Box>
          ))}
        </Box>
      </Box>

...
const SingleCarousel = ({ images }) => {
  ...
  return (
    <Box height={"120vh"} sx={{ position: "relative", overflow: "hidden" }}>
      {images.map((item, n) => (
        <Box 
          sx={{
            position: "absolute",
        ...
        >
          <CardMedia component="img" src={item} sx={{height:"100%" ,  filter:"brightness(0.7)"}}  />
          <Box
            p={3}
            sx={{
              position: "absolute",
      ...
      >
        ...
        >                                                                                    
          <KeyboardArrowLeftIcon style={{ color: grey[50], zIndex:10, fontSize: 30 }} />
        </IconButton>
        <IconButton
          onClick={handleNext}
          onMouseOver={() => setHover(true)}
          onMouseLeave={() => setHover(false)}
        >
          <KeyboardArrowRightIcon fontSize="large" style={{ color: grey[50], zIndex:10, fontSize: 30 }} />
        </IconButton>
      </Box>
    </Box>
  );

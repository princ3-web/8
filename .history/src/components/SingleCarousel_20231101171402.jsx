...
const SingleCarousel = ({ images }) => {
  ...
  return (
    <Box height={"120vh"} sx={{ position: "relative", overflow: "hidden", filter:"brightness(0.7" }}>
      {images.map((item, n) => (
        <Box 
          sx={{
            position: "absolute",
        ...
        >
          <CardMedia component="img" src={item} sx={{height:"100%"}}  />
          <Box
            p={3}
            sx={{
              position: "absolute",
      ...
      >
        ...
        >                                                                                    
          <KeyboardArrowLeftIcon fontSize="large" style={{ color: grey[50], zIndex:10 }} />
        </IconButton>
        <IconButton
          onClick={handleNext}
          onMouseOver={() => setHover(true)}
          onMouseLeave={() => setHover(false)}
        >
          <KeyboardArrowRightIcon fontSize="large" style={{ color: grey[50], zIndex:10 }} />
        </IconButton>
      </Box>
    </Box>
  );
